...
export function renderPreviewVerifyAgentTaskBlock(args: {
  ...
  const lines = [
    ...
    "",
    "SEND-CALL CONTRACT — when the user DOES reply \"send\", call",
    "send_transaction with these args (EVM path):",
    "  - handle: <the same handle>",
    "  - confirmed: true",
    "  - previewToken: <the `previewToken` value from THIS preview_send's",
    "    top-level JSON response — not anything you remember from an",
    "    earlier call on the same handle>",
    "  - userDecision: \"send\"",
    "The previewToken + userDecision pair is the server-side gate that",
    "proves THIS menu was shown to the user and \"send\" was their reply.",
    "Missing/mismatched values are rejected with a clear error — don't",
    "fabricate either. If preview_send was re-called with refresh:true",
    "since you captured the token, the old token is invalid: surface the",
    "fresh hash + this menu again before retrying.",
    "",
