...
export function renderAgentTaskBlock(
  ...
  const lines = [
    ...
    `  5. End your reply with this exact one-line reminder (substitute the`,
    `     short hash):`,
    `       "Before approving on Ledger: confirm the device shows short hash`,
    `        ${shortHash} — reject if it doesn't match."`,
