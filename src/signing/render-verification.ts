...
export function renderAgentTaskBlock(
  ...
  const lines = [
    ...
    `     unprompted:`,
    `       (a) Open the swiss-knife URL in their own browser (the`,
    `           decoder runs in the user's own context, no MCP code on`,
    `           the path — genuinely different trust boundary).`,
    `       (b) Ask you to decode the calldata from your built-in ABI`,
...
export function renderPreviewVerifyAgentTaskBlock(args: {
  ...
  const lines = [
    ...
    "restatement. Wording suggestion:",
    '  "Want me to independently serialize the pinned EIP-1559 tuple and',
    '   check hash(tuple) equals the server\'s preSignHash? This is narrower',
    '   than (b) from prepare time — it catches a (tuple, hash) pair that',
    '   doesn\'t correspond, which the on-device match can\'t detect on its',
    '   own. If you already ran (b), this adds the pair-consistency check',
    '   on top; otherwise I\'ll decode the calldata first as a prerequisite."',
    "",
    ...
    "shared context with either). Wording suggestion:",
    '  "Want me to call get_verification_artifact(handle) and hand you a',
    '   copy-paste JSON block for a second, independent LLM to decode the',
    '   tx from scratch? Paste into a separate chat session (ideally a',
    '   different LLM provider — the point is no shared context with me).',
    '   If the two agents disagree on what the tx does, REJECT. This is',
    '   the strongest of the checks — a second LLM with no shared context',
    '   cannot be coordinated with this one."',
    "",
