...
- **BTC / LTC indexer extensions: `difficulty_timeline` + coinbase pool-tag parsing** — closes the remaining open scope from #233. The two tools that weren't in PR #266's RPC-tier ship because they need indexer-tier extensions, not RPC. Read-only against existing Esplora endpoints; ~300-450 LoC + tests including the curated pool-tag list (rots over time but that's a curation pace issue, not a correctness one). ([plan](./claude-work/plan-incidents-and-utxo-rpc-followups.md) §5)
